<?php 
    ...
    function sendMessage($user_to, $body, $date){
        if($body != ""){
            $userLoggedIn = $this->user_obj->getUsername();

            $query = mysqli_query($this->con, "INSERT INTO messages VALUE('',
    ...
    public function getConvos(){
        ...
        $convos = array();
        // Get UserLoggin user_to, user_from
        $query = mysqli_query($this->con,"SELECT user_to,user_from FROM messages
        ...
        while($row = mysqli_fetch_array($query)){
            ...
            $user_to_push = $row['user_to'] 
                !== $userLoggedIn ? $row['user_to'] : $row['user_from'];

            // print_r($user_to_push);
            
            // all username would be unique
            if(!in_array($user_to_push, $convos)){
                array_push($convos, $user_to_push);
            } 
        }
        // Convos stored username not equals to userloggedin
        foreach($convos as $username){
            ...
            // array_push($details_array, $body) [1];
            // array_push($details_array, $date_time);
             
            $dots = strlen($latest_message_details[1]) >= 12 ? "..." :  "";
            // split into 12 characters
            $body_message = str_split($latest_message_details[1], 12);
            $body_message = $body_message[0] . $dots; // 1234 56789 1011 12...
            
            $return_string .= "
                <a href='messages.php?u=$username'>
                    <div class='user_found_messages'>
                        <div>
                            <img style='border-radius: 5px; margin-right: 5px;' 
                                    src='".$user_found_obj->getProfilePic()."' />
                                    ".$user_found_obj->getUsername()."
                        </div>
                       <span class='timestamp_smaller'id='grey'>".$latest_message_details[2]."</span>
                        <p id='grey' style='margin: 0'>
                            " .$latest_message_details[0]. $body_message. "
                        </p>
                    </div>
                </a>    
            ";
    ...
    public function getLatestMessage($userLoggedIn, $user2){
        ...
        $query = mysqli_query($this->con, "SELECT date, body, user_to FROM messages 
    ...
    public function getConvosDropdownx($data, $limit)
    {
        $page = $data['page'];
        $userLoggedIn = $this->user_obj->getUsername();
        $return_string = "";
        ...
        $set_viewed_query = mysqli_query($this->con, 
            "UPDATE messages SET viewed='yes' WHERE user_to='$userLoggedIn'");
        ...
        foreach($convos as $username){
            ...
            $return_string .= "
                <a href='messages.php?u=$username'> 
								...
								<span class='timestamp_smaller' id='grey'> " . $latest_message_details[2] . "</span>
								<p id='grey' style='margin: 0;'>" . $latest_message_details[0] . $body_message . " </p>
								</div>
				</a>
                 ";
    ...
    public function getConvosDropdown($data, $limit)
    {
        ...
        $set_viewed_query = mysqli_query($this->con,"UPDATE messages 
            SET viewed='yes' WHERE user_to='$userLoggedIn'");
     ...
     public function getUnreadNumber() {
		...
		$query = mysqli_query($this->con, "SELECT * FROM messages 
            WHERE viewed='no' AND user_to='$userLoggedIn'");
