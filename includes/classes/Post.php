...
class Post{
    ...
    public function submitPost($body, $user_to){
		...
		$body = str_replace('\r\n', "\n", $body);
		$body = nl2br($body);
		$check_empty = preg_replace('/\s+/', '', $body); //Deletes all spaces _empty = preg_replace('/\s+/', '', $body); // Deletes all spaces 
        ...
        if($check_empty != ""){
            ...
            $query = mysqli_query($this->con, "INSERT INTO posts VALUES('', 
                '$body','$userLoggedIn', '$user_to','$date_added', 'no','no', '0')");
			...
			$update_query = mysqli_query($this->con, "UPDATE users SET
                num_posts='$num_posts' WHERE username='$userLoggedIn'");
			...
			$no_punctuationBody = preg_replace("/[^a-zA-Z 0-9]+/", "", $body);

            // Predict whether user is posting a url. If so, do not check for trending words
			if(strpos($no_punctuationBody, "height") === false && strpos($no_punctuationBody, "width") === false
				&& strpos($no_punctuationBody, "http") === false && strpos($no_punctuationBody, "youtube") === false){

				// Convert users post (with punctuation removed) into array - split at white space
				$keywords = preg_split("/[\s,]+/", $no_punctuationBody);
				$keywords2 = explode(' ', $no_punctuationBody);
 
                // If the keywords mached to the stopWords, Ignore it.
	...
	public function loadPostsFriendsx() {
		 
		$userLoggedIn = $this->user_obj->getUsername();

        $str = "";

        $data_post_query = mysqli_query($this->con, "SELECT * FROM posts 
            WHERE deleted='no' ORDER BY id DESC");

        if(mysqli_num_rows($data_post_query) > 0) {

            while($row = mysqli_fetch_array($data_post_query)) {

                $post_id = $row['id'];
                $body = $row['body'];
                // User who posted a posts
                ...
                $user_logged_in = new User($this->con, $userLoggedIn);

                if($user_logged_in->isFriend($added_by)){

                    // Logged In user sees his own posts
                    ...
                    $comment_checks = mysqli_query($this->con, "SELECT * FROM comments 
                        WHERE post_id='$post_id'");
                    $comment_checks_num = mysqli_num_rows($comment_checks);
                    ...
                    if($userLoggedIn == $added_by){
                        ...
                        $delete_button = "
                            <button style='float:right' class='delete_button btn-danger' id='post$post_id'>
                    ...
                    $str .= "
                            <div class='status_post' onClick='toggle$post_id()'>
                            ...
                            <div class='post_comment' id='toggleComment$post_id' style='display:none'>
                                <iframe src='comment_frame.php?post_id=$post_id' id='comment_iframe' frameborder='0'></iframe>
                            </div>
                        ...
                        $(document).ready(function (){  
                            $('#post<?php echo $post_id ?>').on('click', function (){
                                bootbox.confirm("Are you sure to delete this post?", function(result) {
                                    $.post("includes/form_handlers/delete_post.php?post_id=<?php echo $post_id ?>",
                                    { result: result });
    ...
    public function loadProfilePosts($username){

        $userLoggedIn = $this->user_obj->getUsername();

        $str = "";

        if($this->user_obj->isFriend($username)){
            
            $data_post_query = mysqli_query($this->con, "SELECT * FROM posts 
                WHERE deleted='no' 
                AND ((added_by='$username' AND user_to='none') OR user_to='$username') 
                ORDER BY id DESC");
            ...
            if(mysqli_num_rows($data_post_query) > 0) {
                ...
                while($row = mysqli_fetch_array($data_post_query)) {
                    ...
                    $user_logged_in = new User($this->con, $userLoggedIn);
                        ...
                        $comment_checks = mysqli_query($this->con, "SELECT * FROM comments where post_id='$post_id'");
                        $comment_checks_num = mysqli_num_rows($comment_checks);
                        ...
                        $str .= "
                                <div class='status_post' onClick='toggle$post_id()'>
                                        ...
                                        <div class='newsfeedPostOptions'>
                                            Comments($comment_checks_num) <iframe src='like.php?post_id=$post_id'scrolling='no'></iframe>
                                        </div>
                                </div>
    ...
    public function getSinglePost($post_id){
		...
		$opened_query = mysqli_query($this->con, "UPDATE notifications 
            SET opened='yes' 
        ...
        if(mysqli_num_rows($data_post_query) > 0) {
           ...
           $row = mysqli_fetch_array($data_post_query);
                ...
                // if($user_logged_in->isFriend($added_by)){
                    ...
                    $str .= "
                            ...
                            <div class='post_comment' id='toggleComment$post_id' style='display:none'>
                                <iframe src='comment_frame.php?post_id=$post_id'
                ...
                // }
                // else{
                //     echo "You cannot view the post of this user. Friend is a must";
                //     return;
                // }
